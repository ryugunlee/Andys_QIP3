...
| Gross Margin ⓝ | 매출총이익률 | 매출총이익 / 매출 |
| Net Margin ⓝ | 순이익률 | 순이익 / 매출 |
...
| Net Debt to Equity ⓝ | 순부채비율 | (이자발생부채 − 현금) / 자기자본 |
| Cash Ratio ⓝ | 현금비율 | 현금 / 유동부채 |
...
| 컬럼 | 의미 | 계산식 |
|---|---|---|
| Inventory Turnover | 재고자산회전율 | 매출원가 / 재고자산 |
| Receivables Turnover | 매출채권회전율 | 매출 / 매출채권 |
| Capex to Revenue | 설비투자 집약도 | \|Capex\| / 매출 |
| Effective Tax Rate | 유효법인세율 | 법인세비용 / 세전이익 |

> ⓝ 표시 10개 팩터는 이번에 **수집 영역에만** 추가된 신규 팩터다. `snapshot_factors`에
> curated 컬럼으로 저장되지만 아직 종합점수 수식·방향성(`analysis/`)에는 연결하지 않았다
> (다음 단계에서 방향성·가중치를 부여할 자리). 두 소스(야후/네이버) 모두 동일한 의미로
> 채운다 — 야후는 대차대조표/손익계산서 원본 라벨, 네이버는 WiseFn 계정코드(ACCODE,
> `collection/constants.py`)로 같은 계정을 읽는다. 비율 팩터라 네이버의 억원 단위는 상쇄된다.
...
각 팩터가 투자 판단에서 무엇을 말해주는지 정리한다. 점수는 절대값이 아니라 모집단 내
상대 순위(2절)로 매겨지므로, 아래 "좋은 방향"은 방향성(HIGHER/LOWER)의 근거를 뜻한다.
ⓝ는 이번에 수집 영역에 추가된 신규 팩터(아직 스코어링 미연결).

### 9.1 밸류에이션 — "지금 주가가 이익/자산 대비 싼가"
- **PER/PBR/PSR/PCR**: 각각 이익·순자산·매출·영업현금흐름 대비 주가 배수. 낮을수록 저평가로
  본다(역수 방향). PCR/PSR은 적자·회계이익 왜곡에 PER보다 덜 흔들린다.
- **EV/EBITDA·EV/Revenue**: 시가총액이 아니라 순부채까지 반영한 기업가치(EV) 기준 배수라
  부채 구조가 다른 기업을 비교할 때 PER보다 공정하다.
- **PEGR**: PER를 이익성장률로 나눠 "성장까지 감안하면 비싼가"를 본다.
- **PFCR**: 잉여현금흐름 대비 주가. 회계이익이 아닌 실제 남는 현금 기준이라 이익 품질에 강하다.
- **NCAV**: (유동자산−유동부채)/시가총액. 그레이엄식 청산가치 안전마진으로, 높을수록 자산이
  시총을 방어한다.

### 9.2 수익성 — "버는 힘이 강한가"
- **ROE/ROA/ROC**: 자기자본·총자산·투하자본 대비 이익률. 자본을 얼마나 효율적으로 이익으로
  바꾸는지를 본다. 높을수록 좋다.
- **GPTOA(매출총이익/총자산)**: 노비-마르크스식 수익성 지표로, 자산 대비 원초적 이익창출력을
  보며 장기 수익률 예측력이 높은 것으로 알려져 있다.
- **Operating / Gross / Net Margin ⓝ**: 매출 1원이 각각 영업이익·매출총이익·순이익으로 얼마나
  남는가. Gross는 제품 경쟁력(가격결정력), Operating은 본업 효율, Net은 세금·금융비용까지
  반영한 최종 남는 힘을 본다. 세 마진의 간격이 벌어지면 판관비·금융비용·일회성 손익이 이익을
  갉아먹고 있다는 신호다. 높을수록 좋다.

### 9.3 효율성·비용구조 ⓝ — "자산을 얼마나 빠르게 돈으로 바꾸는가"
- **Inventory Turnover(재고자산회전율) = 매출원가/재고자산**: 재고가 1년에 몇 번 팔려나가는지.
  높을수록 재고가 묶이지 않고 빠르게 매출로 전환된다(재고 진부화·과잉재고 위험이 낮다).
  단, 업종차가 커서(예: 반도체 vs 소프트웨어) 섹터 내 비교가 특히 중요하다.
- **Receivables Turnover(매출채권회전율) = 매출/매출채권**: 외상매출을 얼마나 빨리 현금으로
  회수하는가. 높을수록 회수가 빠르고 대손·매출 부풀리기 위험이 낮다.
- **Capex to Revenue(설비투자 집약도) = |Capex|/매출**: 매출 대비 설비투자 부담. 성장 투자의
  크기이자 동시에 현금 소모 강도라, 무조건 높/낮음이 좋은 게 아니라 업종·성장국면과 함께 읽어야
  한다(그래서 방향성 부여를 보류했다).

### 9.4 재무 건전성 — "빚을 감당할 수 있는가"
- **Debt to Equity / Asset to Equity**: 부채비율·재무 레버리지. 레버리지는 호황엔 이익을 키우지만
  불황엔 위험을 키운다.
- **Current Ratio(유동비율)·Quick Ratio(당좌비율 ⓝ)**: 유동부채를 유동자산으로 갚을 수 있는가.
  당좌비율은 그중 **팔아야 현금이 되는 재고자산을 뺀** 더 보수적인 지표라, 재고 비중이 큰 기업의
  단기 지급능력을 유동비율보다 정직하게 본다. 둘 다 높을수록 안전하다.
- **Cash Ratio(현금비율 ⓝ) = 현금/유동부채**: 유동자산 중에서도 즉시 지급 가능한 현금만으로 단기
  부채를 얼마나 덮는가. 가장 엄격한 유동성 지표다.
- **Net Debt to Equity(순부채비율 ⓝ) = (이자발생부채−현금)/자기자본**: 보유 현금을 빼고 남는
  '순수한 빚'의 부담. 음수면 현금이 이자부 부채보다 많은 **순현금(net cash)** 상태로, 재무적으로
  매우 견고하다는 뜻이다(예: 검증 시 삼성전자 −0.07). 낮을수록 좋다.
- **Interest Ratio(이자보상배율)·Coverage Ratio(영업현금흐름/부채)**: 영업으로 번 이익·현금으로
  이자와 부채 원금을 감당할 여력. 높을수록 부도 위험이 낮다.
- **Debt Growth(부채 증가율)**: 빚이 빠르게 늘면 경계 신호(작을수록 좋음, 음수 반전).

### 9.5 이익 품질 — "장부상 이익이 진짜 현금인가"
- **ARP(발생액 비율) = (순이익−영업현금흐름)/시가총액**: 회계이익이 현금흐름보다 크면 발생액이
  많다는 뜻으로, 이익의 지속성이 낮을 수 있다(작을수록 좋음, 음수 반전).
- **Depreciation Capex Ratio**: 감가상각 대비 설비투자. 자산을 갉아먹는지/재투자하는지의 균형.
- **Effective Tax Rate(유효법인세율 ⓝ) = 법인세비용/세전이익**: 실제로 낸 세금 비율. 지나치게
  낮으면 세금 혜택·일회성 요인으로 순이익이 부풀려졌을 수 있어 이익 품질을 교차 점검하는 데
  쓴다(적자 기업은 분모가 음수라 의미가 없으므로 흑자 기업 위주로 해석). 방향성은 좋/나쁨이
  일률적이지 않아 보류했다.

### 9.6 주주환원·지분
- **Dividend Yield·Buyback Yield**: 배당·자사주매입으로 주주에게 돌려주는 총수익률. 높을수록 좋다.
- **Dividend to Income·Buyback to Income**: 이익 대비 환원 비중(배당성향 등).
- **Insiderpercent·Institutionpercent·Insider Buy Ratio**: 내부자·기관 지분과 내부자 순매수. 내부자
  매수는 경영진의 자기 회사에 대한 확신 신호로 읽는다(야후 전용, 네이버는 결측).

### 9.7 수급·모멘텀
- **3M/6M/1Y Ratio**: 기간별 주가 수익률. 추세 지속(모멘텀)을 본다.
- **Turnover(거래대금/시총)·Overheat(단기/중기 과열도)·Volatility**: 거래 활발도와 과열·변동성.
  과열 구간은 되돌림 위험을, 낮은 유동성은 진입/청산 비용을 뜻한다.

> 신규 10개 팩터에 아직 방향성·가중치를 부여하지 않은 이유: 마진·회전율은 업종 편차가 크고,
> 설비투자 집약도·유효법인세율은 단순히 높/낮음으로 우열을 가릴 수 없다. 스코어링 연결은
> 섹터 내 비교(2.3의 `Sec` 모집단) 반영 방식과 함께 다음 단계에서 설계한다.
