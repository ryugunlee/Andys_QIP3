...
## collection/financial_trend.py
- `evaluate_uptrend(values_by_period)`: {회계기간: 값} → "Y"/"N"/None(4개년 미만은 판정 보류).
  "5년간 꾸준히 올랐는가"를 순증가(마지막>첫해) + 하락 연수 제한(1~2년 허우적 허용)으로 판정.
  야후(보통 4개년)·네이버(5개년) 양쪽 데이터 깊이 차이에 맞춰 5개년↔4개년의 허용 하락
  연수를 다르게 둔다(`ALLOWED_DOWN_YEARS_FULL=2` / `_SHORT=1`). 소스 무관 순수 함수.

## collection/stock_base.py
- `CURATED_COLUMNS`: 표의 컬럼명 ↔ `BaseStock` 속성명 매핑 (원래 `stock.py`에 있던 것을 이동).
  신규 팩터 10종(Operating/Gross/Net Margin, Net Debt to Equity, Cash Ratio,
  Capex to Revenue, Inventory Turnover, Quick Ratio, Effective Tax Rate,
  Receivables Turnover)에 이어 `Revenue Trend (5Y)`/`Operating Income Trend (5Y)`
  (Y/N/결측 텍스트, `financial_trend.evaluate_uptrend` 결과)가 추가됐다.
...
- `series_adapter.py`: `FinancialSeries(statements, source)` — `to_financial_statement_rows()`가
  ...
  **억원→원 단위 환산**을 여기서 전부 흡수한다(환산이 없으면 FCF Yield가 1억 배 틀린다).
  ...
  WiseFn에서 못 찾으면 **모바일 API(`finance_annual`)의 한글 항목명으로 한 번 더 시도**한다 —
  금융사는 손익계산서 계정 트리가 달라 당기순이익(203170)이 아예 없기 때문이다.
...
- `parsers.py`: `parse_number`/`parse_won_amount`(한글 숫자 표기 "23.04배"/"1,666조 1,894억" 파싱),
  ...
  `series_by_accode(statements, accode)`: `get_wise_value`처럼 기간 하나가 아니라 **전체 회계기간**을
  {기간: 값}으로 반환(컨센서스 제외) — 5년 오름세 판정(`financial_trend.evaluate_uptrend`)에 쓴다.
- `naver_stock.py`: `NaverStock(BaseStock)`. `fetch()`가 basic/일봉(5년)/integration/finance-annual/
  ...
  - `_compute_wise_factors`: WiseFn 원본 계정 금액(억원)으로 Operating Cashflow/PCR/PFCR/
    ...
    읽기 + 나머지)가 담당.
  - `_compute_financial_trend_factors`(`compute_curated_factors`가 `_compute_wise_factors` 다음에
    호출): `wise_income_statement`에서 `series_by_accode`로 매출·영업이익 전체 회계기간을 뽑아
    `financial_trend.evaluate_uptrend`로 `revenue_trend_5y`/`operating_income_trend_5y`를 채운다
    (억원 단위 그대로 써도 됨 — 추세 판정은 상대 비교라 스케일 무관).
...
- `build_group_indices(conn)`: 시장/섹터/산업 지수를 산출·저장하고 저장 행 수를 반환.
  ...
  - **검증(2026-09-07)**: KOSPI 시총 상위 60종목으로 산출한 지수와 실제 KOSPI(KS11)의
    일간수익률 **상관계수 0.9946**(898거래일). 구간 수익률은 자체 지수가 더 높은데,
    표본이 시총 상위 60종목이라 반도체 대형주 비중이 실제 KOSPI(약 800종목)보다
    크기 때문이다 — 산출식 오류가 아니라 모집단 차이다. 상대강도는 어차피 **우리
    유니버스 안에서의 비교**라 이 성질이 오히려 맞다.
    별도 픽스처로 시총가중 수동 계산 일치와 run 경계 look-ahead 부재도 확인했다.
...
- `qip4_gate.py`: `attach_gate_columns(df)` — S1~S3·상환연수로 `QIP4 Stability Gate`("PASS"/"FAIL")와
  ...
  - **자산형(은행·보험·증권·지주)은 현금흐름 조건(S1~S3·상환연수) 대신 전용 조건을 쓴다**:
    F1(3년 중 순손실 2년 이상), F2(자기자본비율 < 4%). 은행은 이자비용이 조달원가라
    이자보상배율 1 미만이 정상이고 영업현금흐름도 대출 잔액 변동으로 흔들린다.
    면제만 하면 관문이 비므로 대체 조건을 둔 것이다. 자산 품질 경고는 자산형에도 적용.
...
- `base.py`: `StockRepository` Protocol — `good_stocks(limit)`, `qip3_stocks(limit)`,
  `top_by_market_cap(region, limit)`, `iter_stock_details()`, `chart_bundle(ticker, market)`,
  `search_entries()`, `market_counts()`, `group_scores(group_type)`, `updated_date()`.
  빌더는 이 계약만 의존한다. `qip3_stocks`는 QIP3 선별(get_goodstock2), CSV 폴백은 빈 목록.
...
- `db_repository.py`: `DuckDbStockRepository` — **기본 구현체**. 주식 DB **2개(KR/US)**를 순회해
  ...
  `qip3_stocks`는 `get_goodstock2`를 `_load_runs`로 통합해 QIP3 종합점수 내림차순 반환(캐시).
