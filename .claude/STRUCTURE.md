...
- 차트용 dataclass: `PricePoint`(일봉 한 점: date/close/volume/foreign_rate),
  `AnnualFinancials`(연간 매출·영업이익·순이익 — 없으면 None), `StockCharts`(prices·annual 묶음,
  데이터 없는 계열은 빈 리스트→템플릿에서 섹션 숨김).
...
- `financial_series.py`: `annual_financials_from_df(df, source)` — 재무제표 long DataFrame에서
  연간 매출·영업이익·순이익 3계열을 뽑는다. 소스별 항목 식별자(naver=ACCODE 접두사
  200000/201370/203170, yahoo=Total Revenue/Operating Income/Net Income)를 이 파일에 캡슐화,
  is_consensus(컨센서스)는 제외.
- `db_repository.py`: `DuckDbStockRepository` — **기본 구현체**. 주식 DB **2개(KR/US)**를 순회해
  ...
  `get_financial_statements`를 재사용, 종목별 반복 조회를 위해 DB 연결을 캐시한다.
...
- `detail_pages.py`: `build_detail_pages()` — 전 종목 상세. `ticker_filename()` sanitize,
  ...
  병렬 배열 d/c/v로 압축)와 `_financial_table()`(연간 실적 표 — 영업이익률·전년比 매출성장률을
  빌더가 미리 계산, 최근 연도부터 정렬)로 변환해 템플릿에 전달. 데이터 없으면 둘 다 None.
...
- `base.html`(공통 레이아웃, 컨텍스트: root/active_page/updated_date, `{% block scripts %}`로
  페이지별 JS 선택 로드), `index.html`, `stocks.html`, `stock_detail.html`, `sectors.html`,
  partials(`_header`/`_stock_card`/`_top_cap_table`/`_score_bar`/`_metric_table`/`_group_table`/
...
- `stock_detail.html`: 가격 섹션(`chart_data.prices`가 있을 때만, 기간 토글 1/3/6/12/60개월 +
  ...
  `renderBars` 애니메이션 + `financial_table` 표 동반). `chart_data`는 `<script id="chart-data"
  type="application/json">`로 임베드, 있을 때만 `static/charts.js` 로드.
- `static/style.css`: 토스풍 라이트 테마, 카드/표/점수미터/CSS 탭, 모바일 720px 브레이크포인트.
  ...
  `.reveal-block`/`.bar-legend`/`.bar-chart`/`.fin-table`(실적).
- `static/charts.js`: 외부 라이브러리 없는 바닐라 SVG 차트 (search.js와 같은 자체구현 철학).
  ...
  `renderBars()` — 매출/영업이익/순이익 그룹 막대, `<details>` toggle 이벤트에서 최초 1회
  렌더 + 0선에서 자라는 rAF 트윈 애니메이션. `init()`이 `#chart-data`의 JSON을 파싱해 배정.
