...
## 23. 재무 실적 시계열은 연간만 존재, 분기 데이터는 수집·저장 어디에도 없음

종목 상세 페이지에 매출·영업이익·순이익 막대그래프를 추가하면서(presentation/repository/
financial_series.py) 확인한 사실: `financial_statements` 테이블에는 **연간 데이터만** 있다.
한국은 `NaverStock.fetch()`가 `fetch_finance_annual`만 호출하고(네이버 WiseFn에는 분기
엔드포인트가 있는 것으로 보이나 미구현), 미국은 yfinance의 연간 `financials`/`cashflow`/
`balance_sheet`만 사용한다(quarterly_financials 등 분기 속성은 미사용).

사용자가 분기 단위 막대그래프도 요청했으나, 이번 작업 범위는 연간으로 한정하고 분기는
후속 작업으로 분리하기로 확인받았다. 분기를 추가하려면: (1) 미국은 yfinance
`quarterly_financials` 등을 추가로 수집해 `financial_statements`에 statement_type을
구분해(예: `financials_q`) 저장하면 비교적 쉽다. (2) 한국은 네이버 WiseFn 분기
엔드포인트를 새로 조사·구현해야 해서 범위가 크다. 표현 계층(`renderBarChart`,
`annual_financials_from_df`)은 연/분기 어느 쪽이든 같은 모양(period, revenue,
operating_income, net_income)이면 재사용 가능하도록 설계해두었다.
