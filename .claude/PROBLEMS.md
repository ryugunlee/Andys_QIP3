...
## 37. 삼성전자 파일럿(4회 실측) 뒤 남은 문제

- **Q7·Q8·Q9는 quick 섹션(II·VI·X)만으로 근거가 얕다.** 배당의 영업CF 내 충당·영업권 손상은 III(현금흐름표·
  주석), 국가별 연결 매출은 주석 30(부문별 보고)에만 있다. Opus deep(III 포함)은 이걸 다 찾았지만 $2.30.
  III 중 필요한 주석(현금흐름표·자본금·부문별 보고·특수관계자·영업권)만 뽑는 2차 분할을 만들면 DeepSeek
  quick으로도 채울 수 있을 것이다 — 다음 과제.
- **같은 문서에서 모델마다 다른 대용값을 쓴다.** Q9 단일 국가 비중을 Sonnet 22%(국내 오독)·DeepSeek 30%
  (별도 기준 중국)·Opus 39.9%(연결 미주 대륙)로 냈다. 필드 설명을 고쳤지만, 재계산 항목은 raw 값의 출처
  (연결/별도, 국가/대륙)를 함께 받는 편이 안전하다.
...
- 관측값 파일이 `qualitative/observations/<티커>.json` 하나라 티어·provider별 결과를 나란히 두려면 손으로
  복사해야 한다(이번엔 scratchpad에 보존). 파일명에 티어·모델을 넣을지 결정 필요.
- ★ 플래그·DB 저장은 이 환경에 DuckDB가 없어 여전히 미실행. EDGAR는 클라우드 IP 차단으로 미검증.
- 같은 원문을 5분 안에 다시 보내면 Anthropic 프롬프트 캐시가 먹혀 입력이 약 1/10 비용이 된다(실측: 캐시
  읽기 77,940 토큰). DeepSeek도 캐시 히트 가격이 있으나 이번엔 확인하지 않았다.
