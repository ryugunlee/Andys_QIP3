...
## 37. 첫 실측(삼성전자 quick)에서 드러난 정성 규칙·파이프라인의 남은 문제

- **Q1(확정 이익)이 수주형이 아닌 사업을 구조적으로 F로 만든다.** 삼성전자는 공시된 수주가 Tesla 파운드리
  계약뿐이라 연매출 대비 1% → 16점. 메모리·가전·소비재처럼 수주잔고 개념이 없는 업종에서 Q1은 "낮음"이
  아니라 "해당 없음"이어야 한다. 후보: ① 업종 판단으로 결측 처리(가중 재배분) ② 반복매출 비중으로
  치환 ③ 자산형처럼 섹터군별 치환표. 개발자 결정 대기.
- **Q7(자본배분)·Q8·Q9는 quick 섹션(II·VI·X)만으로 근거가 얕다.** 배당의 영업CF 내 충당 여부는 III(현금
  흐름표)에만 있어 null. deep 티어 또는 III 섹션의 필요한 절만 뽑는 2차 분할이 필요하다.
- **Q8 최대 고객 비중은 추정이다.** 사업보고서는 "5대 매출처 약 15%"만 공시해 LLM이 0.10으로 추정했고
  코드가 그 수치로 재계산했다. 추정값을 raw로 받는 것을 허용할지, `weak`로 표시할지 정해야 한다.
- OpenAI 호환 provider(`compat_client.py`)는 키가 없어 미검증. DeepSeek 계열의 한국어 토크나이저 효율과
  컨텍스트 한도(128K 안팎)가 quick 섹션 8.6만 자에 맞는지 실측이 필요하다.
- 같은 원문을 5분 안에 다시 보내면 프롬프트 캐시가 먹혀 입력이 약 1/10 비용이 된다(실측: 캐시 읽기
  77,940 토큰). 규칙·프롬프트를 고치며 반복 실행할 때 이 창을 활용한다.
