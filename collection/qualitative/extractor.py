...
- LLM 1회 호출로 요청한 항목을 전부 채점한다. provider(Anthropic / OpenAI 호환)는 호출 함수로 주입받는다.
- 인용 대조: 추출된 quote가 원문에 실제로 있는지 문자열로 확인한다. 없으면 `weak`.
  이것이 환각을 막는 유일한 기계적 장치다(로케이터만 요구하면 로케이터도 지어낸다).
- 응답은 항목별로 전체 스키마에 대조해 어긋난 항목만 결측으로 바꾼다(`response_check.py`).
...
from collection.qualitative.prompts import SYSTEM_PROMPT, build_output_schema, build_request, build_schema
from collection.qualitative.response_check import normalize_payload, validate_items
from collection.qualitative.sources import SourceDocument, save_raw_response
...
# provider 중립 호출 계약: (system, document, request_text, schema) → 항목 코드를 키로 하는 dict.
# Anthropic은 `llm_client.extract_structured`, OpenAI 호환은 `compat_client.compat_extract_structured`를
# functools.partial로 묶어 넘긴다.
...
def extract_observations(
    ...
    raw_payload = call(
        system=SYSTEM_PROMPT, document=document, request_text=build_request(specs), schema=build_output_schema(specs),
    )
    print(f"[qualitative] 원본 응답 저장: {save_raw_response(ticker, asof, raw_payload)}")
    payload, replaced = validate_items(normalize_payload(raw_payload), build_schema(specs))
    if replaced:
        print(f"[qualitative] 스키마 위반으로 결측 처리한 항목: {', '.join(replaced)}")
