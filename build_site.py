...
from presentation.builders.site_builder import EmptySiteError, build_site
...
def main() -> None:
    ...
    except EmptySiteError as error:
        # 데이터 공급이 끊긴 실행은 "성공한 빈 사이트"가 아니라 실패다 —
        # 호출하는 CI 스크립트가 커밋을 건너뛰도록 0이 아닌 코드로 끝낸다.
        sys.exit(f"[build_site] 오류: {error}")
